...
def plot_pr_curves(model, test_loader, num_classes, device, class_counts, title, file_name):   
    ...
    with open('source1_pr_curve_data.pkl', 'wb') as f:
        pickle.dump(data, f)
...
def plot_roc_curves(model, test_loader, num_classes, device, class_counts, title, file_name):
    ...
    with open('source1_roc_curve_data.pkl', 'wb') as f:
        pickle.dump(data, f)    
...
if __name__ == "__main__":
    batch_size = 32
    hidden_size = 128
    model_name = 'baseline_classifier_iscx_vpn_nomargin_9.pth'
    title_pr = "Class-Wise PR Curves (No Margin) - ISCX-VPN"
    title_auc = "AUC-ROC Curve - ISCX-VPN"
    file_name_pr = "fig-pr-curves-iscx-vpn(nomargin).png"
    file_name_auc = "fig-roc-curves-iscx-vpn(nomargin).png"

    model_path = os.path.join(SAVED_MODELS_DIR, 'baseline_classifier_iscx_vpn_nomargin_9.pth')
    ...
    plot_pr_curves(model, test_loader, num_classes, device, iscx_vpn_class_counts, title_pr, file_name_pr)    
    # plot_roc_curves(model, test_loader, num_classes, device, iscx_vpn_class_counts, title_auc, file_name_auc)